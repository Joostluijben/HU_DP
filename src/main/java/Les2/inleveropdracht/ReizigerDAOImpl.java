...
public class ReizigerDAOImpl extends OracleBaseDao implements ReizigerDAO {
    ...
    public List<Reiziger> findAll() throws SQLException, ClassNotFoundException {
        ...
        while (result.next()) {
            ...
            Date gbdatum = Date.valueOf(result.getString(5).split(" ")[0]);
    ...
    public Reiziger save(Reiziger reiziger) throws SQLException {
        ...
        statement.setString(1, String.valueOf(reiziger.getReizigersID()));
    ...
    public Reiziger update(Reiziger reiziger) throws SQLException, ClassNotFoundException {
        ...
        for (Reiziger rz : findAll()) {
            ...
            if (reiziger.getReizigersID() == rz.getReizigersID()) {
                ...
                statement.setString(5, String.valueOf(rz.getReizigersID()));
                statement.execute();
            }
        }
        return reiziger;
    ...
    public boolean delete(Reiziger reiziger) throws SQLException, ClassNotFoundException {
        ...
        //loop through all the persons using the findAll method
        for (Reiziger rz : findAll()) {
            if (rz.getReizigersID() == reiziger.getReizigersID()) {
                ...
                stament.setString(1, String.valueOf(reiziger.getReizigersID()));
                stament.execute();
            }
        }
    ...
    public List<OVchipkaart> getOVchipkaarten(Reiziger rz) throws SQLException, ClassNotFoundException {
        //create a new ovi to get all the OV cards
        OVchipkaartDAOImpl ovi = new OVchipkaartDAOImpl();
        List<OVchipkaart> ovs = new ArrayList<>();
        //loop through all the OV cards, if the persons ID matches add the OV card
        for (OVchipkaart ov : ovi.findall()) {
            if (ov.getReizigerID() == rz.getReizigersID()) {
                ovs.add(ov);
            }
        }
